...
- **`:` palette commands**: Public commands are registered after WASM init; authenticated commands are registered via `fzt.addCommands()` only after `checkAuth` returns true. Current public entry: `ambience mode` (fade out homepage chrome and show only the ambience canvas until Escape). Current authenticated entries: `sync` (in-tab re-fetch via `triggerManualSync` → shows dot on diff, title-bar status otherwise; action string is `refresh` to dodge fzt-frontend's filesystem-backed `sync`), `edit` (enter HTML tree editor), `logout` (clear JWT + reload), `copy yaml` / `paste yaml` (round-trip the tree through the clipboard).
...
Uses the shared auto-init client from the `ambience` repo, served from our OWN origin as a version-pinned vendored bundle (the same approach chess-tactics uses). Homepage is meant to be a full subscriber to the same authority stream as `ambience.romaine.life`: if ambience is open on one computer and homepage on another, both should show the same active effect, scene, config, and visible phase as closely as browser/network scheduling allows. Treat this like a clock-sync goal, not a decorative background. The client reads `snapshotData.Type` from the server, looks up the constructor in `AmbienceSim.effects[type]`, restores authoritative snapshots, and follows config/trigger/scene commands from `https://ambience.romaine.life/events`. Keystrokes captured on homepage are POSTed to `ambience.romaine.life/entropy` on a 2s throttle; the shared atmosphere's RNG drifts with user input.

- **Vendored runtime, not runtime-loaded**: `frontend/ambience/` holds a pinned snapshot fetched from `ambience.romaine.life` — `sim.js`, `client.js`, `wasm_runtime.js`, `wasm_exec.js`, and the FULL all-effects `ambience.wasm` (committed; ~6.5 MB), plus `manifest.json` recording the pinned sizes. `index.html` loads `/ambience/sim.js` + `/ambience/client.js` and sets `data-ambience-wasm-url` / `-wasm-exec-url` / `-runtime-url` on the canvas to the vendored copies, so NOTHING about the runtime is fetched from ambience's origin at page load — only the SSE stream (`/events`, `/snapshot`) points there. This replaced the old frozen `ambience-sim.js` / `ambience-client.js` copies, which were an ancient era (10 Hz / 200×100) that desynced against the current 60 Hz / 320×180 authority and mis-rendered effects.
- **Full wasm, not rain-scoped**: homepage subscribes to ambience's DEFAULT rotating world (bare paths), which can broadcast ANY effect — so it vendors `ambience.wasm` (all effects), unlike chess-tactics which vendors the rain-only `ambience-rain.wasm` for its single-effect `/chess` world. The connect-time capability handshake (`servedEffects` in each snapshot; client asserts `AmbienceSim.effects[name]` exists for each) fails loudly rather than mis-rendering if the vendored build ever drifts from what the world serves.
- **Re-vendoring**: run `npm --prefix frontend run vendor:ambience` (optionally `AMBIENCE_BASE=…` for a non-prod origin) after ambience ships a new client version, then commit the updated `frontend/ambience/`. The fingerprint path lists in `build-and-deploy.yaml` and `docker-build-check.yaml` include the `frontend/ambience` directory, so a re-vendor triggers a rebuild.
- **Graceful absence**: if the vendored client fails to load, `body.ambience-on` is never set and the terminal keeps its Catppuccin bg — no broken render path.
...
- **Ambience client vendored + pinned (fixes "weird effects")** — homepage was running a STALE hand-vendored ambience client (`frontend/ambience-sim.js` / `ambience-client.js`, a 10 Hz / 200×100 era) against the live 60 Hz / 320×180 authority, and fetching the WASM from `ambience.romaine.life` at runtime — both caused desync / mis-rendered effects. Migrated onto chess-tactics' vendored-client approach: new `scripts/vendor-ambience.mjs` fetches `sim.js`, `client.js`, `wasm_runtime.js`, `wasm_exec.js`, and the FULL `ambience.wasm` into `frontend/ambience/` with a `manifest.json` pinning sizes (committed). `index.html` loads `/ambience/sim.js` + `/ambience/client.js` and sets `data-ambience-wasm-url` / `-wasm-exec-url` / `-runtime-url` so the runtime loads from the vendored copy, not ambience's origin. Homepage subscribes to the rotating default world (bare paths), so it vendors the all-effects `ambience.wasm` (not chess's rain-only artifact); the connect-time `servedEffects` handshake validates capability. Deleted the old frozen copies + `scripts/sync-ambience.mjs` (sibling-repo file-copy sync); `package.json` `sync:ambience`/`check:ambience` → `vendor:ambience`. CI fingerprint path lists updated to track the `frontend/ambience` directory.
