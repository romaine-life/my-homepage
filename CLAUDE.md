...
- **Cookie auth endpoints not live** — the `/auth/code` and `/auth/callback` endpoints were added to the routes package and published, but the API is still returning 404 for them. The root cause is the publish → API lockfile gap: the first push published v0.1.12 (auth strip, pre-cookie), the second push (cookie auth) didn't trigger a separate publish run (GitHub Actions deduplication). A manual no-op push triggered v0.1.13, but the API's `npm ci` used the old lockfile pointing to v0.1.12. Manual `npm update` + push got the lockfile to v0.1.12 but v0.1.13 may not have been picked up. The `homepagelogin` AT command fails at the code exchange step because the endpoint doesn't exist on the deployed API.
- **Publish → API lockfile gap** — the publish-routes pipeline dispatches `dependency-updated` to the API repo, but the API's CI runs `npm ci` from the old lockfile. The new routes version is published but never installed without a manual lockfile update. Needs a pre-build step in the API workflow to run `npm update` on dispatch events.
- **Pipeline dependency diagram** — new `/pipelines` route in infra-diagram showing the cross-repo pipeline dependency chain (fzt → my-homepage → api) with the lockfile gap visualized as a broken link.
