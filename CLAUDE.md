...
Ambient rain overlay behind the fzt bookmark terminal. `<canvas id="ambience-canvas" data-ambience>` sits at the top of `<body>` (before `#app`). Canvas is `position: fixed; z-index: 0; pointer-events: none`; `#app` bumped to `z-index: 1`. `--fzt-bg` overridden to `transparent` under `body.ambience-on` so rain mingles with terminal text — drops appear in empty glyph space, text glyphs and cells with explicit bg render opaque on top. If the ambience JS fails to fetch, `body.ambience-on` is never set and the terminal keeps its Catppuccin bg — no broken render path.

Uses the shared auto-init client from the `ambience` repo. Vendored files: `frontend/ambience-sim.js` + `frontend/ambience-client.js` (sourced from `ambience/cmd/ambience/web/`). Client is effect-agnostic — reads `snapshotData.Type` from the server, looks up the constructor in `AmbienceSim.effects[type]`, so future effects (sand/fire/tetris) drop in with no homepage changes. Keystrokes captured and POSTed to `ambience.romaine.life/entropy` on a 2s throttle; the shared atmosphere's RNG drifts with user input.

Keep vendored files in sync when upstream `ambience` sim.js/client.js change — copy from `/d/repos/ambience/cmd/ambience/web/`.
