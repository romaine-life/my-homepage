...
Keep vendored files in sync when upstream `ambience` `cmd/ambience/web/sim.js` or `client.js` changes: run `npm --prefix frontend run sync:ambience` with `AMBIENCE_REPO` set if the ambience checkout is not a sibling of this repo. The current browser effect implementations come from the ambience Go/WASM runtime loaded by `client.js`, not from vendored per-effect JS files. `npm --prefix frontend run check:ambience` fails when the vendored files are stale. A stale vendored client can silently degrade homepage back into a partial subscriber that ignores newly promoted live effects.
